...
4. Type `yourname-toolname/index.html` — the slash creates the folder
5. Paste, then **Commit changes**
6. Repeat for `meta.json` (copy from `_template/meta.json`, fill in your details)
...
4. Paste the result into a new folder under your own name
5. Update `meta.json`

Do not edit someone else's folder. Make your own copy.
...
`status` is either `working` or `wip`. Be honest — `wip` is not a failure, it's an invitation.

If `meta.json` is missing or broken, your tool still shows up. The build never fails because of one bad file.
...
├── gilley-molar-mass/      ← example: simple
├── gilley-titration-sim/   ← example: more involved
├── scripts/
